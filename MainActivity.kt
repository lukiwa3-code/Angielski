...
class VocabularyStore(private val context: Context) {
    ...
    fun loadSettings(): AppSettings {
        val modeName = prefs.getString("sourceMode", SourceMode.GITHUB.name) ?: SourceMode.GITHUB.name

        return AppSettings(
            caseSensitive = prefs.getBoolean("caseSensitive", false),
            acceptTypos = prefs.getBoolean("acceptTypos", true),
            sourceMode = runCatching { SourceMode.valueOf(modeName) }.getOrDefault(SourceMode.GITHUB),
        ...
        runCatching {
            ...
            for (i in 0 until array.length()) {
                ...
                if (polish.isNotBlank() && english.isNotBlank()) {
                    result += WordEntry(polish = polish, english = english)
                }
            }
        }
...
fun WordTrainerApp() {
    ...
    val store = remember { VocabularyStore(context.applicationContext) }

    var screen by remember { mutableStateOf<AppScreen>(AppScreen.Home) }
    var settings by remember { mutableStateOf(store.loadSettings()) }
    var dictionaries by remember { mutableStateOf<List<DictionaryBundle>>(emptyList()) }
    var loading by remember { mutableStateOf(false) }
    var status by remember { mutableStateOf("") }
    var creativeRefresh by remember { mutableIntStateOf(0) }
    ...
    val folderPicker = rememberLauncherForActivityResult(
        ...
        if (uri != null) {
            ...
            scope.launch {
                loading = true
                status = "Ładowanie słowników z folderu..."
                dictionaries = loadDictionaries(context, settings)
                loading = false
        ...
        Box(
            ...
            when (val current = screen) {
                AppScreen.Home -> HomeScreen(
                    ...
                    loading = loading,
                    status = status,
                    onReload = { reloadDictionaries() },
                    onSettings = { screen = AppScreen.Settings },
                    onLearn = { dictionary -> screen = AppScreen.Learn(dictionary) },
                    onTest = { screen = AppScreen.TestSetup }
                )
                ...
                AppScreen.TestSetup -> TestSetupScreen(
                    ...
                    onBack = { screen = AppScreen.Home },
                    onStart = { selected ->
                        screen = AppScreen.TestRun(selected)
                    }
                )

                is AppScreen.TestRun -> TestRunScreen(
                    dictionaries = current.dictionaries,
                    settings = settings,
                    store = store,
                    onExit = {
                        creativeRefresh++
...
fun SettingsScreen(
    ...
    var caseSensitive by remember { mutableStateOf(settings.caseSensitive) }
    var acceptTypos by remember { mutableStateOf(settings.acceptTypos) }
    var sourceMode by remember { mutableStateOf(settings.sourceMode) }
    var githubIndexUrl by remember { mutableStateOf(settings.githubIndexUrl) }
    var folderUri by remember { mutableStateOf(settings.folderUri) }
    ...
    LazyColumn(
        ...
        item {
            SettingSwitchRow(
                ...
                checked = acceptTypos,
                onCheckedChange = { acceptTypos = it }
            )
        }
        ...
        item {
            Row(
                ...
                RadioButton(
                    ...
                    onClick = { sourceMode = SourceMode.GITHUB }
                )
                Text("GitHub / raw URL")
            }
            ...
            Row(
                ...
                RadioButton(
                    ...
                    onClick = { sourceMode = SourceMode.FOLDER }
                )
        ...
        item {
            ...
            Text(
                text = folderUri.ifBlank { "Nie wybrano folderu" },
...
fun LearningScreen(
    ...
    val streaks = remember(dictionary.id) {
    ...
    val hiddenWords = remember(dictionary.id) {
        mutableStateMapOf<String, Boolean>()
    }

    var seriesNumber by remember(dictionary.id) { mutableIntStateOf(1) }
    var roundWords by remember(dictionary.id) { mutableStateOf(dictionary.words.shuffled()) }
    var currentIndex by remember(dictionary.id) { mutableIntStateOf(0) }
    var answer by remember(dictionary.id) { mutableStateOf("") }
    var message by remember(dictionary.id) { mutableStateOf("") }
    var completed by remember(dictionary.id) { mutableStateOf(dictionary.words.isEmpty()) }
    ...
    fun submitAnswer() {
        if (completed || roundWords.isEmpty()) return
    ...
    LazyColumn(
        ...
        item {
            ...
            val progress = if (total == 0) 0f else (mastered + hidden).toFloat() / total.toFloat()
        ...
        if (currentWord != null) {
            ...
            item {
                Row(
                    ...
                    OutlinedButton(onClick = { hideCurrentWord() }) {
...
fun TestSetupScreen(
    ...
    LazyColumn(
        ...
        item {
            val selectedDictionaries = allDictionaries.filter { selected[it.id] == true }
...
fun TestRunScreen(
    ...
    val wrongAnswers = remember { mutableStateListOf<TestWrongAnswer>() }
        ...
        when (settings.sourceMode) {
            ...
            SourceMode.FOLDER -> {
                if (settings.folderUri.isBlank()) {
                    ...
                    loadFolderDictionaries(context, Uri.parse(settings.folderUri))
                }
            }
        }
    }
    ...
    return text
        ...
        .mapNotNull { line ->
            ...
            if (separatorIndex > 0) {
                ...
                val url = line.trim()
                val name = url.substringAfterLast("/").substringBeforeLast(".").ifBlank {
                    "słownik"
                }
